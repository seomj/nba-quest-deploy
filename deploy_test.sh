...
function create_lb(){
  ...
  if [ $? -ne 0 ]; then
    echo "========== SA 생성 실패 =========="
    curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"SA 생성 실패"}' -H "Content-Type: application/json" $API_ENDPOINT
    exit 1
  fi
  ...
  if [ $? -ne 0 ]; then
    ...
    curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"Load Balancer Controller 설치에 실패했습니다."}' -H "Content-Type: application/json" $API_ENDPOINT
    exit 1
  fi
}
...
for EKS in $EKS_LIST; do
  if [ "$EKS" == "$AWS_EKS_NAME" ]; then
    ...
    if [ $? -ne 0 ]; then
      ...
      curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"'$AWS_EKS_NAME'의 kubeconfig로 업데이트할 수 없습니다."}' -H "Content-Type: application/json" $API_ENDPOINT
      exit 1
    fi
    # eks가 있다면 true
    EKS_EXIST=true
...
if [ "$EKS_EXIST" == false ]; then
  ...
  curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"'$AWS_EKS_NAME'를 찾을 수 없습니다."}' -H "Content-Type: application/json" $API_ENDPOINT
  exit 1
fi
...
if [ "$LB_STATUS" != True ]; then
  ...
  curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"Load balancer controller를 사용할 수 없습니다."}' -H "Content-Type: application/json" $API_ENDPOINT
  exit 1
else
...
if [ "$NS_EXIST" == false ]; then
  ...
  if [ $? -ne 0 ]; then
    ...
    curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"namespace 생성 실패"}' -H "Content-Type: application/json" $API_ENDPOINT
    exit 1
  else
...
EOF
  ...
  if [ $? -ne 0 ]; then
    echo "========== secert 실패 =========="
    curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"secert 실패"}' -H "Content-Type: application/json" $API_ENDPOINT
    exit 1
  fi
  done
fi
...
if [ $? -ne 0 ]; then
  echo "========== service 실패 =========="
  curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"service 실패"}' -H "Content-Type: application/json" $API_ENDPOINT
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "========== deployment 실패 =========="
  curl -i -X POST -d '{"id":'$ID',"progress":"deployment","state":"failure","emessage":"deployment 실패"}' -H "Content-Type: application/json" $API_ENDPOINT
  exit 1
fi
